...
import {
  ...
  Repeat2, Phone, Mail, Wrench, Download, ExternalLink, X
} from "lucide-react";
...
export default function App() {
  ...
  async function openDocument(order, type) {
    ...
    try {
      ...
      setDocPreview(current => { if (current?.url) URL.revokeObjectURL(current.url); return { url, title: isOffer ? "Коммерческое предложение" : "Заказ-наряд", filename: `${isOffer ? "KP" : "Zakaz-naryad"}-${order.id}.pdf` }; });
      setScreen("document-preview"); goTop();
  ...
  if (screen === "home") {
    ...
    const nextBooking = orders.filter(o=>o.booking_status==="confirmed"&&o.scheduled_at&&new Date(o.scheduled_at)>new Date()).sort((a,b)=>new Date(a.scheduled_at)-new Date(b.scheduled_at))[0];
    return <div className="app"><Header subtitle="Личный кабинет"/><main className="gfHome">
      ...
      {nextBooking&&<section className="gfNextBooking" onClick={()=>{setSelectedOrder(nextBooking);go("order-details")}}><div><span className="gfEyebrow">БЛИЖАЙШАЯ ЗАПИСЬ</span><h3>{formatDate(nextBooking.scheduled_at,true)}</h3><p>{vehicleName(nextBooking.vehicle)}</p></div><CalendarDays size={26}/></section>}
  ...
  if (screen === "document-preview" && docPreview) return <div className="app gfDocumentScreen"><header className="gfDocumentHeader"><button type="button" onClick={()=>{setScreen("order-details");goTop()}}><ChevronLeft size={22}/>Назад</button><strong>{docPreview.title}</strong><button type="button" aria-label="Закрыть документ" onClick={()=>{setScreen("order-details");goTop()}}><X size={22}/></button></header><main className="gfDocumentMain"><iframe title={docPreview.title} src={docPreview.url}/></main><div className="gfDocumentActions"><button className="continueButton" onClick={()=>window.open(docPreview.url,"_blank")}><ExternalLink size={18}/>Открыть PDF</button></div></div>;
  ...
  if (screen === "order-details" && selectedOrder) {
    ...
    return <div className="app"><Header subtitle={`Заказ №${selectedOrder.id}`}/><main><button className="linkButton" onClick={()=>go("orders")}><ChevronLeft size={18}/>Все заказы</button>
      ...
      {selectedOrder.status==="done"&&<section className="section"><div className="serviceCard gfDoneSummary"><CircleCheckBig size={34}/><div><small>ЗАКАЗ ЗАВЕРШЁН</small><h2>Автомобиль готов</h2><p>Работы выполнены. Ниже доступны итоговая стоимость, фотографии и документы по заказу.</p><strong>{formatPrice(selectedOrder.final_price??selectedOrder.preliminary_price)}</strong></div></div></section>}
      {selectedOrder.scheduled_at&&selectedOrder.booking_status==="confirmed"&&<section className="section"><div className="serviceCard gfInfoRow"><CalendarDays/><div><small>Подтверждённая запись</small><h3>{formatDate(selectedOrder.scheduled_at,true)}</h3></div></div></section>}<section className="section"><div className="sectionHeader"><div><h2>Запись</h2><p>{selectedOrder.booking_status==="confirmed"?"Запись подтверждена":selectedOrder.booking_status==="reschedule_requested"?"Запрошен перенос":selectedOrder.booking_status==="scheduled"?"Менеджер предложил другое время":selectedOrder.booking_status==="cancelled"?"Запись отменена":"Ожидает решения менеджера"}</p></div><CalendarDays size={22}/></div>{selectedOrder.requested_at&&<div className="serviceCard gfInfoRow"><Clock3/><div><small>Желаемое время клиента</small><h3>{formatDate(selectedOrder.requested_at,true)}</h3></div></div>}{selectedOrder.scheduled_at&&selectedOrder.booking_status==="scheduled"&&<><div className="serviceCard gfInfoRow gfManagerOffer"><CalendarDays/><div><small>Менеджер предложил</small><h3>{formatDate(selectedOrder.scheduled_at,true)}</h3></div></div><button className="continueButton gfWide" disabled={bookingBusy} onClick={async()=>{setBookingBusy(true);try{await callClient("confirm_booking",{order_id:selectedOrder.id});await bootstrap();setSelectedOrder(o=>({...o,booking_status:"confirmed"}));}catch(e){setOrderError(e.message)}finally{setBookingBusy(false)}}}>Подтвердить запись на {formatDate(selectedOrder.scheduled_at,true)}</button></>}{selectedOrder.booking_status==="confirmed"&&<div className="gfBookingConfirmed"><Check size={18}/> Запись подтверждена: {formatDate(selectedOrder.scheduled_at,true)}</div>}<div className="gfBookingMove">{!rescheduleOpen?<button className="linkButton gfWide" onClick={()=>setRescheduleOpen(true)}>Перенести</button>:<div className="gfReschedulePanel"><small>Выберите новое желаемое время</small><input className="gfInput" type="datetime-local" value={preferredAt} onChange={e=>setPreferredAt(e.target.value)}/><div className="gfRescheduleActions"><button className="linkButton" onClick={()=>{setRescheduleOpen(false);setPreferredAt("")}}>Отмена</button><button className="continueButton" disabled={bookingBusy||!preferredAt} onClick={async()=>{setBookingBusy(true);try{await callClient("request_reschedule",{order_id:selectedOrder.id,preferred_at:new Date(preferredAt).toISOString()});await bootstrap();setSelectedOrder(o=>({...o,requested_at:new Date(preferredAt).toISOString(),booking_status:"reschedule_requested"}));setPreferredAt("");setRescheduleOpen(false);}catch(e){setOrderError(e.message)}finally{setBookingBusy(false)}}}>Отправить запрос</button></div></div>}</div></section>
      ...
      <section className="section"><div className="sectionHeader"><div><h2>История заказа</h2><p>Основные события для клиента</p></div><Clock3 size={22}/></div><div className="gfTimeline"><div className="gfTimelineItem done"><span>✓</span><div><b>Заявка создана</b><small>{formatDate(selectedOrder.created_at,true)}</small></div></div>{selectedOrder.booking_status==="confirmed"&&selectedOrder.scheduled_at&&<div className="gfTimelineItem done"><span>✓</span><div><b>Запись подтверждена</b><small>Запись: {formatDate(selectedOrder.scheduled_at,true)}</small>{selectedOrder.customer_confirmed_at&&<small>Подтверждено: {formatDate(selectedOrder.customer_confirmed_at,true)}</small>}</div></div>}{st.step>=1&&<div className="gfTimelineItem done"><span>✓</span><div><b>Заказ согласован</b></div></div>}{st.step>=2&&<div className="gfTimelineItem done"><span>✓</span><div><b>Производство</b></div></div>}{st.step>=3&&<div className="gfTimelineItem done"><span>✓</span><div><b>Установка</b></div></div>}{st.step>=4&&<div className="gfTimelineItem done"><span>✓</span><div><b>Работы завершены</b></div></div>}</div></section>
      <section className="section"><div className="sectionHeader"><div><h2>Документы</h2><p>PDF по вашему заказу</p></div><FileText size={22}/></div><div className="gfDocGrid"><button disabled={documentBusy} onClick={()=>openDocument(selectedOrder,"offer")}><FileText size={18}/><span><b>Коммерческое предложение</b><small>Открыть документ</small></span></button><button disabled={documentBusy} onClick={()=>openDocument(selectedOrder,"work_order")}><FileText size={18}/><span><b>Заказ-наряд</b><small>Открыть документ</small></span></button></div>{orderError&&<p className="gfErrorText">{orderError}</p>}</section>
      {["done","cancelled"].includes(selectedOrder.status)&&<section className="section"><button className="continueButton gfWide" onClick={()=>repeatOrder(selectedOrder)}><Repeat2 size={18}/>Записаться снова</button></section>}
