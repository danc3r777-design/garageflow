...
export default function CrmApp() {
  ...
  function printOrderDocument(type) {
    ...
    const rows = (selectedOrder.items || []).map((item, index)=>`<tr><td>${index+1}</td><td>${escapeDocument(item.service_name)}</td><td>${escapeDocument(item.material||"")}</td><td>${Number(item.quantity||1)}</td><td>${formatPrice(item.price)}</td><td>${formatPrice(Number(item.price||0)*Number(item.quantity||1))}</td></tr>`).join("");
    ...
    w.document.write(`<!doctype html><html lang="ru"><head><meta charset="UTF-8"><title>${title} №${selectedOrder.id}</title><style>body{font-family:Arial,sans-serif;color:#171717;margin:40px;font-size:14px}.head{display:flex;justify-content:space-between;gap:30px;border-bottom:2px solid #111;padding-bottom:18px}.brand{font-size:28px;font-weight:800}.muted{color:#666;line-height:1.5}h1{font-size:24px;margin:28px 0 6px}.grid{display:grid;grid-template-columns:1fr 1fr;gap:18px;margin:22px 0}.box{border:1px solid #ddd;border-radius:10px;padding:14px}.box strong{display:block;margin-bottom:6px}table{width:100%;border-collapse:collapse;margin-top:24px}th,td{border:1px solid #ddd;padding:9px;text-align:left}th{background:#f4f4f4}.total{text-align:right;font-size:20px;font-weight:800;margin-top:20px}.footer{margin-top:40px;padding-top:18px;border-top:1px solid #ddd;color:#666;white-space:pre-line}.sign{display:grid;grid-template-columns:1fr 1fr;gap:50px;margin-top:55px}.sign div{border-top:1px solid #555;padding-top:7px}@media print{body{margin:18mm}.no-print{display:none}}</style></head><body><div class="head"><div><div class="brand">${escapeDocument(companySettings.company_name||"GarageFlow")}</div><div class="muted">${escapeDocument(companySettings.legal_name||"")}<br>${companySettings.inn?`ИНН ${escapeDocument(companySettings.inn)}`:""} ${companySettings.kpp?` · КПП ${escapeDocument(companySettings.kpp)}`:""}<br>${escapeDocument(companySettings.address||"")}</div></div><div class="muted">${escapeDocument(companySettings.phone||"")}<br>${escapeDocument(companySettings.email||"")}</div></div><h1>${title} №${selectedOrder.id}</h1><div class="muted">от ${new Date().toLocaleDateString("ru-RU")}</div><div class="grid"><div class="box"><strong>Клиент</strong>${escapeDocument(getCustomerName(selectedOrder.customer))}<br>${escapeDocument(selectedOrder.customer?.phone||"")}</div><div class="box"><strong>Автомобиль</strong>${escapeDocument(getVehicleName(selectedOrder.vehicle))}<br>${selectedOrder.vehicle?.license_plate?`Госномер: ${escapeDocument(selectedOrder.vehicle.license_plate)}`:""}${selectedOrder.vehicle?.vin?`<br>VIN: ${escapeDocument(selectedOrder.vehicle.vin)}`:""}</div></div><table><thead><tr><th>№</th><th>Работа</th><th>Материал</th><th>Кол.</th><th>Цена</th><th>Сумма</th></tr></thead><tbody>${rows}</tbody></table><div class="total">Итого: ${formatPrice(total)}</div>${!isQuote&&selectedOrder.scheduled_at?`<p><strong>Дата записи:</strong> ${escapeDocument(formatDate(selectedOrder.scheduled_at))}</p>`:""}${selectedOrder.manager_comment?`<p><strong>Комментарий:</strong> ${escapeDocument(selectedOrder.manager_comment)}</p>`:""}<div class="footer">${escapeDocument(companySettings.bank_details||"")}\n${escapeDocument(companySettings.document_footer||"")}</div>${!isQuote?'<div class="sign"><div>Исполнитель / подпись</div><div>Заказчик / подпись</div></div>':""}<script>window.onload=()=>setTimeout(()=>window.print(),250)<\/script></body></html>`);
    w.document.close();
  }
