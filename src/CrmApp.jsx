    ...
    try {
      if (status === "done") {
        ...
        if ((check.warnings || []).length) {
          const ok = window.confirm(`Перед завершением заказа:\n\n${check.warnings.map((x)=>`• ${x}`).join("\n")}\n\nВсё равно перевести заказ в «Готово»?`);
          if (!ok) return;
        }
      }
